...
/**
 * MultiSet then Get then Delete for keys.
 *
 ...
 */
public final class MultiSetGetDeleteServlet extends HttpServlet {
  private static final Logger logger = Logger.getLogger(MultiSetGetDeleteServlet.class.getName());
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    for (int i = 0; i < iterationCount; ++i) {
      ...
      if (memcache.getAll(keys).isEmpty()) {
        logger.severe("Memcache get_multi failed.");
        writer.fail();
        return;
      }

      if (memcache.deleteAll(keys).isEmpty()) {
        logger.severe("Memcache delete_multi failed.");
        writer.fail();
        return;
      }
