 ...
 *
 */
public class AppEngineSerialization {
   ...
   * that value.  Used as the return from {@link AppEngineSerialization#serialize}.
   */
  ...
  private static final String MYCLASSNAME = AppEngineSerialization.class.getName();
  ...
  static {
    try {
      ...
      throw new TranscoderException("No SHA-1 algorithm, cannot hash keys for memcache", ex);
    }
  }
  ...
  private AppEngineSerialization() {
    // non-instantiable
  }
  ...
  public static Object deserialize(byte[] value, int flags)
    ...
    switch (flagval) {
      ...
      case BOOLEAN:
        if (value.length != 1) {
          throw new TranscoderException("Cannot deserialize Boolean: bad length", null);
        }
        switch (value[0]) {
          case TRUE_VALUE:
            return Boolean.TRUE;
        ...
        throw new TranscoderException("Cannot deserialize Boolean: bad contents", null);
      ...
      case LONG:
        ...
        switch (flagval) {
          ...
          case LONG:
            return val;
          default:
            throw new TranscoderException("Cannot deserialize number: bad contents", null);
        }
  ...
  private static byte[] makePbKey(Object key) throws IOException {
    ...
    // Fallback case for types other than strings, bytearrays, base types, or for when the they
  ...
  public static ValueAndFlags serialize(Object value)
    ...
    // TODO(ozarov): as all types except Serialization are final we could
